...
def aesthetic_score(device):
    from cdm.rewards.aesthetic_scorer import AestheticScorer
...
def clip_score(device):
    from cdm.rewards.clip_scorer import ClipScorer
...
def hpsv2_score(device):
    from cdm.rewards.hpsv2_scorer import HPSv2Scorer
...
def hpsv3_score(device):
    from cdm.rewards.hpsv3_scorer import HPSv3Scorer
...
def pickscore_score(device):
    from cdm.rewards.pickscore_scorer import PickScoreScorer
...
def imagereward_score(device):
    from cdm.rewards.imagereward_scorer import ImageRewardScorer
...
def geneval_score(device):
    from cdm.rewards.gen_eval import load_geneval
...
def ocr_score(device):
    from cdm.rewards.ocr import OcrScorer

    scorer = OcrScorer(device=device)
...
def multi_score(device, score_dict):
    ...
    _fn._score_fns = score_fns
    return _fn


def get_scorer_models(reward_fn):
    """Extract nn.Module scorer models from a multi_score reward function.

    Returns a list of (score_name, scorer) tuples for scorers that are
    ``torch.nn.Module`` instances and can be wrapped by ``accelerator.prepare()``.
    """
    score_fns = getattr(reward_fn, '_score_fns', {})
    models = []
    for score_name, fn in score_fns.items():
        scorer = getattr(fn, '_scorer', None)
        if scorer is not None and isinstance(scorer, torch.nn.Module):
            models.append((score_name, scorer))
    return models
